...
void AUList::PutItem(CCFraudRecord item) { //This function assumes the "IsFull" condition hasn't been met.
...
CCFraudRecord AUList::GetNextItem() {
...
void AUList::PrintList() { //simple function to print a list's items in stored order
  for (int loc=0; loc<length; loc++) {
  	CCFraudRecord curitem=ListItems[loc];
	std::cout<< "Rec " << loc << ": " <<curitem.time<<", "<<curitem.amount<<", "
	<<curitem.fraudclass<<std::endl;
  }
}
