...
void BST::Insert (TNode* &curnode, Laptop_Data newitem) { //Helper function to insert an item into (sub-)tree denoted by curnode
  if (curnode==NULL) {
    curnode = new TNode;
	curnode->item = newitem;
	curnode->right = NULL;
	curnode->left = NULL;
  } else if (newitem.Laptop_ID<curnode->item.Laptop_ID)
    Insert(curnode->left,newitem);
  else
    Insert(curnode->right,newitem);
}
  
Laptop_Data BST::FindItem(TNode* curnode, int gitem) { //Helper function to locate an item in (sub-)tree denoted by curnode
  if (curnode == NULL)
    //return -1;
    cout << "Item not found" << endl;
  else if (gitem == curnode->item.Laptop_ID)
    return curnode->item;
  else if (gitem < curnode->item.Laptop_ID)
...
void BST::DeleteNode(TNode* &delnode) { //Helper function to delete an actual node
  ...
  if ((delnode->left) == NULL) { //If left child is null, we can replace with right child (whether NULL or not!)
    ...
    Laptop_Data replaceitem = predecessor->item;
    delnode->item = replaceitem;
    Delete(delnode->left, replaceitem.Laptop_ID);
  }     
...
void BST::Delete(TNode* &curnode, int ditem) { //Helper function to delete an item in (sub-)tree denoted by curnode
  if (curnode == NULL)
    throw;
  else if (ditem < curnode->item.Laptop_ID)
...
void BST::PutItem(Laptop_Data newitem) {
  Insert(root,newitem);
}  

Laptop_Data BST::GetItem(int gitem) {
...
void BST::PrintNodes(TNode* curnode) { // Prints items in (sub-)tree in sorted order
  if (curnode != NULL) {
    ...
    // print every item from the laptop_data struct
    std::cout << "Rec " << curnode->item.Laptop_ID << " :" << curnode->item.Company << " ,"  << curnode->item.Product << " ," << curnode->item.TypeName << " ," << curnode->item.Inches << " ," << curnode->item.ScreenResolution << " ," << curnode->item.Cpu << " ," << curnode->item.Ram<< " ," << curnode->item.Memory << " ," << curnode->item.Gpu << " ," << curnode->item.OpSys << " ," << curnode->item.Weight << " ," << curnode->item.Price_euros << std::endl;
